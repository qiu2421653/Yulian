...
import com.njkj.yulian.ui.activity.dialog.CaptionDialog;
...
public class ExampleFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		imageview.setLayoutParams(new FrameLayout.LayoutParams(
				FrameLayout.LayoutParams.WRAP_CONTENT, width / 16 * 9));
		...
		bottom_text.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					...
					if (top == width / 16 * 9 - view.getHeight() - 1) {
						if (z > 0) {
							top = 0;
						} else {
							top = width / 16 * 9 - view.getHeight() - 1;
						}
					}
					break;
				default:
	...
	public void moveViewByLayout(View view, int rawX, int rawY) {
		...
		int shang = rawY - dip2px(getActivity(), getStatusBarHeight())
				- view.getHeight() / 2;
		if (shang > 1 && shang < (width / 16 * 9 - view.getHeight() - 1)) {
		...
		} else if (shang > width / 16 * 9 - view.getHeight() - 1) {
			top = width / 16 * 9 - view.getHeight() - 1;
		}
	}
	...
	public ClipImageLayout getFragmentView() {
	...
	public void onClick(View v) {
		...
		case R.id.xuanzhuan1:
			this.getFragmentView().xuanzhuan(width / 2, width / 16 * 9 / 2);
			break;
		default:
			addfont();
			break;
	...
	private void addfont() {
		...
		new CaptionDialog(getActivity(), R.style.MyDialogStyle,new Isdialog() {
			
			@Override
			public void result(boolean type,String result) {
				if(type==true){
					intitData(result);
				}
			}
