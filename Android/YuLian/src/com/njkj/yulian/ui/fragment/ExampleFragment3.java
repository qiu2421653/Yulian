...
@SuppressLint("NewApi")
public class ExampleFragment3 extends Fragment implements OnClickListener {
	View view;
	int width;
		...
		bottom_text.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					...
					if (top == width  - view.getHeight() - 1) {
						if (z > 0) {
							top = 0;
						} else {
							top = width  - view.getHeight() - 1;
						}
					}
					break;
				default:
	...
	public void moveViewByLayout(View view, int rawX, int rawY) {
		...
		int shang = rawY - dip2px(getActivity(), getStatusBarHeight())
				- view.getHeight() / 2;
		if (shang > 1 && shang < (width  - view.getHeight() - 1)) {
		...
		} else if (shang > width  - view.getHeight() - 1) {
			top = width  - view.getHeight() - 1;
		}
	}
	...
	public ClipImageLayout getFragmentView3() {
	...
	public void onClick(View v) {
		...
		case R.id.xuanzhuan1:
			this.getFragmentView3().xuanzhuan(width / 2, width / 2);
			break;
		default:
			addfont();
			break;
