...
import com.njkj.yulian.ui.adapter.CommentRewardAdapter;
import com.njkj.yulian.ui.adapter.CommentRewardAdapter.OnRewardCallBack;
import com.njkj.yulian.ui.adapter.CommentRewardAdapter.onHpicCallBack;
 ...
 * @version 1.0 ==============================
 */
public class TopicRewardActivity extends BaseActivity implements
		OnLoadMoreListener, OnRefreshListener, onHpicCallBack {
	...
	CommentRewardAdapter commentAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initData();
		initOnClick();
		System.out.println("hPic:" + mConfigDao.getString("hPic"));
	}
	...
	private void initMessage() {
		...
		commentAdapter = new CommentRewardAdapter(mContext, detailEntity,
				replyToCommentListener, replyToReplyListener);
		commentAdapter.setOnRewardCallBack(onRewardCallBack);
		commentAdapter.setmCallBack(this);
		swipe_target.setAdapter(commentAdapter);
		showSuccess();
		getTopicForks();
		getTopicComments();
	...
	/** 打赏 */
	public OnRewardCallBack onRewardCallBack = new OnRewardCallBack() {
		@Override
	...
	@Override
	public void onHpicClick(int position) {
		// 其他人爱情
	...
	// 获取帖子内容
	private void FsGetTopic(final String topicId) {
		String userID = mConfigDao.getString("userID");
		topicController.getTopic(getString(R.string.FsGetTopic), topicId,
	...
	private void getTopicForks() {
		...
		topicController.getTopicForks(getString(R.string.FsGetTopicForks),
				forkCount, forkPage, detailEntity.infoId, userID,
				new SimpleCallback() {
					@Override
					public void onCallback(Object data) {
						if (data == null) {
	...
	private void setTopicFork(int isFork) {
		...
		userController.setTopicFork(getString(R.string.FsSetTopicFork), userID,
				detailEntity.infoId, isFork, 0, new SimpleCallback() {
					@Override
					public void onCallback(Object data) {
						if (data == null) {
							showShortToast(getString(R.string.error));
