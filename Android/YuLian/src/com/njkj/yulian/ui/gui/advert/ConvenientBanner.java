...
public class ConvenientBanner extends LinearLayout {
	...
	private List<AdvertEntity> mDatas;
	...
	public ConvenientBanner setPages(List<AdvertEntity> datas) {
		this.mDatas = datas;
			...
			viewPager
					.setPageTransformer(
							...
							(PageTransformer) Class.forName(
									"com.njkj.yulian.ui.gui.advert.transforms."
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if (ev.getAction() == MotionEvent.ACTION_UP) {
			// HotFragment.isCan=true;
			// 开始翻页
			if (canTurn)
				startTurning(autoTurningTime);
		} else if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			// HotFragment.isCan=false;
			// 停止翻页
			if (canTurn)
				stopTurning();
		}
