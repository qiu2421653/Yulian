...
package com.njkj.yulian.ui.gui.togglebutton;
...
public class Spring {
	...
	Spring(BaseSpringSystem springSystem) {
		if (springSystem == null) {
			throw new IllegalArgumentException(
					"Spring cannot be created outside of a BaseSpringSystem");
		}
	 ...
	 * displacement ratio. This also updates the start value of the Spring.
	 * 
	 * @param currentValue
	...
	void advance(double realDeltaTime) {
		...
		if (isAtRest && mWasAtRest) {
			/*
			 * begin debug Log.d(TAG, "bailing out because we are at rest:" +
			 * getName()); end debug
			 */
			return;
		}

		// clamp the amount of realTime to simulate to avoid stuttering in the
		// UI. We should be able
		...
		/*
		 * begin debug long startTime = System.currentTimeMillis(); int
		 * iterations = 0; end debug
		 */
		...
		while (mTimeAccumulator >= SOLVER_TIMESTEP_SEC) {
			...
			if (mTimeAccumulator < SOLVER_TIMESTEP_SEC) {
				// This will be the last iteration. Remember the previous state
				// in case we need to
				// interpolate
			...
			// Perform an RK4 integration to provide better detection of the
			// acceleration curve via
			// sampling of Euler integrations at 4 intervals feeding each
			// derivative into the calculation
			// of the next and taking a weighted sum of the 4 derivatives as the
			// final output.

			// This math was inlined since it made for big performance
			// improvements when advancing several
			// springs in one pass of the BaseSpringSystem.

			// The initial derivative is based on the current velocity and the
			// calculated acceleration
			aVelocity = velocity;
			aAcceleration = (tension * (mEndValue - tempPosition)) - friction
					* velocity;

			// Calculate the next derivatives starting with the last derivative
		...
		// End the spring immediately if it is overshooting and overshoot
		// clamping is enabled.
		// Also make sure that if the spring was considered within a resting
		// threshold that it's now
		// snapped to its end value.
		if (isAtRest() || (mOvershootClampingEnabled && isOvershooting())) {
			// Don't call setCurrentValue because that forces a call to
			// onSpringUpdate
		...
		/*
		 * begin debug long endTime = System.currentTimeMillis(); long
		 * elapsedMillis = endTime - startTime; Log.d(TAG, "iterations:" +
		 * iterations + " iterationTime:" + elapsedMillis + " position:" +
		 * mCurrentState.position + " velocity:" + mCurrentState.velocity +
		 * " realDeltaTime:" + realDeltaTime + " adjustedDeltaTime:" +
		 * adjustedDeltaTime + " isAtRest:" + isAtRest + " wasAtRest:" +
		 * mWasAtRest); end debug
		 */

		// NB: do these checks outside the loop so all listeners are properly
