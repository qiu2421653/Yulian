 ...
 * @Package com.njkj.yulian.core.crash
 * 
 * @Description: UncaughtException处理,跳转到错误页面
 * 
 * @date 2016-6-24 上午11:37:46
 * 
 * @version 1.0 ==============================
 */
@SuppressLint("NewApi")
public final class CustomActivityOnCrash {
	...
	private static WeakReference<Activity> lastActivityCreated = new WeakReference<Activity>(
			null);
			...
			if (context == null) {
				...
				if (oldHandler != null
						&& oldHandler.getClass().getName()
								.startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
					Log.e(TAG,
							"You have already installed CustomActivityOnCrash, doing nothing!");
				} else {
					if (oldHandler != null
							&& !oldHandler.getClass().getName()
						...
						public void uncaughtException(Thread thread,
							...
							if (hasCrashedInTheLastSeconds(application)) {
								Log.e(TAG,
										...
										throwable);
								if (oldHandler != null) {
									oldHandler.uncaughtException(thread,
											throwable);
									return;
								}
							} else {
								...
								if (isStackTraceLikelyConflictive(throwable,
								...
								} else if (launchErrorActivityWhenInBackground
										|| !isInBackground) {

									final Intent intent = new Intent(
											application, errorActivityClass);
									...
									intent.putExtra(EXTRA_STACK_TRACE,
											stackTraceString);
									intent.putExtra(
											EXTRA_RESTART_ACTIVITY_CLASS,
											restartActivityClass);
									intent.putExtra(EXTRA_SHOW_ERROR_DETAILS,
											showErrorDetails);
									intent.putExtra(EXTRA_EVENT_LISTENER,
											eventListener);
							...
							final Activity lastActivity = lastActivityCreated
									.get();
								...
								.registerActivityLifecycleCallbacks(new Application.ActivityLifecycleCallbacks() {
									...
									public void onActivityCreated(
										...
										if (activity.getClass() != errorActivityClass) {
											// Copied from ACRA:
											// Ignore activityClass because we
											// want the last
											// application Activity that was
											// started so that we can
	...
	public static boolean isShowErrorDetailsFromIntent(Intent intent) {
		return intent.getBooleanExtra(
				CustomActivityOnCrash.EXTRA_SHOW_ERROR_DETAILS, true);
	}

	/**
	 * Given an Intent, returns the drawable id of the image to show on the
	...
	public static int getDefaultErrorActivityDrawableIdFromIntent(Intent intent) {
		return intent.getIntExtra(
				CustomActivityOnCrash.EXTRA_IMAGE_DRAWABLE_ID,
	 ...
	 */
	public static String getAllErrorDetailsFromIntent(Context context,
			Intent intent) {
		...
		// Ideally, we should not use this method at all... It is only formatted
		// this way because of coupling with the default error activity.
		// We should move it to a method that returns a bean, and let anyone
		// format it as they wish.
	...
	public static Class<? extends Activity> getRestartActivityClassFromIntent(
			Intent intent) {
	...
	public static EventListener getEventListenerFromIntent(Intent intent) {
		Serializable serializedClass = intent
				.getSerializableExtra(CustomActivityOnCrash.EXTRA_EVENT_LISTENER);
	 ...
	 * @param intent
	 *            The Intent. Must not be null.
	 * @deprecated You should use restartApplicationWithIntent(activity, intent,
	 *             eventListener). If you don't want to use an eventListener,
	 *             just pass null.
	 */
	@Deprecated
	public static void restartApplicationWithIntent(Activity activity,
			Intent intent) {
	 ...
	 * Given an Intent, restarts the app and launches a startActivity to that
	 * intent. The flags NEW_TASK and CLEAR_TASK are set if the Intent does not
	 * have them, to ensure the app stack is fully cleared. If an event listener
	 * is provided, the restart app event is invoked. Must only be used from
	 * your error activity.
	 * 
	 * @param activity
	 *            The current error activity. Must not be null.
	 ...
	 *            The Intent. Must not be null.
	 * @param eventListener
	 *            The event listener as obtained by calling
	 *            getEventListenerFromIntent.
	 */
	public static void restartApplicationWithIntent(Activity activity,
			Intent intent, EventListener eventListener) {
	 ...
	 * @param activity
	 *            The current error activity. Must not be null.
	 * @deprecated You should use closeApplication(activity, eventListener). If
	 *             you don't want to use an eventListener, just pass null.
	 */
	@Deprecated
	...
	/**
	 * Closes the app. If an event listener is provided, the close app event is
	 * invoked. Must only be used from your error activity.
	 * 
	 * @param activity
	 *            The current error activity. Must not be null.
	 * @param eventListener
	...
	/**
	 * Defines if the error activity must shown the error details button. Set it
	 * to true if you want to show the full stack trace and device info, false
	 * if you want it to be hidden. The default is true.
	 */
	...
	/**
	 * Defines which drawable to use in the default error activity image. Set
	 * this if you want to use an image other than the default one. The default
	 * is R.drawable.customactivityoncrash_error_image (a cute upside-down bug).
	 */
	public static void setDefaultErrorActivityDrawable(
	 ...
	 */
	public static void setErrorActivityClass(
	 ...
	 * close instead.
	 */
	public static void setRestartActivityClass(
			Class<? extends Activity> restartActivityClass) {
		CustomActivityOnCrash.restartActivityClass = restartActivityClass;
	}

	/**
	 * Returns the event listener to be called when events occur, so they can be
	 * reported by the app as, for example, Google Analytics events.
	 * 
	...
	/**
	 * Sets an event listener to be called when events occur, so they can be
	 * reported by the app as, for example, Google Analytics events. If not set
	 * or set to null, no events will be reported.
	 * 
	 * @param eventListener
	 *            The event listener.
	 * @throws IllegalArgumentException
	 *             if the eventListener is an inner or anonymous class
	 */
		...
		do {
			...
			for (StackTraceElement element : stackTrace) {
				if ((element.getClassName()
						.equals("android.app.ActivityThread") && element
						.getMethodName().equals("handleBindApplication"))
	 ...
	 */
	private static String getBuildDateAsString(Context context,
	 ...
	 * AndroidManifest with intent filter <action
	 * android:name="cat.ereza.customactivityoncrash.RESTART" />, if it cannot
	 * find them, then it will get the default launcher. If there is no default
	 * launcher, this returns null.
	 * 
	 * @param context
	 *            A valid context. Must not be null.
	 ...
	 *         found
	 */
	private static Class<? extends Activity> guessRestartActivityClass(
			Context context) {
	 ...
	 * <action android:name="cat.ereza.customactivityoncrash.RESTART" />, If
	 * there is no activity with that intent filter, this returns null.
	 * 
	 * @param context
	 *            A valid context. Must not be null.
	...
	private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(
			Context context) {
		Intent searchedIntent = new Intent().setAction(
				INTENT_ACTION_RESTART_ACTIVITY).setPackage(
				context.getPackageName());
		List<ResolveInfo> resolveInfos = context.getPackageManager()
				.queryIntentActivities(searchedIntent,
						PackageManager.GET_RESOLVED_FILTER);

		if (resolveInfos != null && resolveInfos.size() > 0) {
			ResolveInfo resolveInfo = resolveInfos.get(0);
			try {
				return (Class<? extends Activity>) Class
	 ...
	 * with intent filter <action
	 * android:name="cat.ereza.customactivityoncrash.ERROR" />, if it cannot
	 * find them, then it will use the default error activity.
	 * 
	 * @param context
	 *            A valid context. Must not be null.
	 ...
	 *         if not found
	 */
	private static Class<? extends Activity> guessErrorActivityClass(
			Context context) {
		Class<? extends Activity> resolvedActivityClass;
	...
	/**
	 * INTERNAL method used to get the first activity with an intent-filter
	 * <action android:name="cat.ereza.customactivityoncrash.ERROR" />, If there
	 * is no activity with that intent filter, this returns null.
	 * 
	 * @param context
	 *            A valid context. Must not be null.
	 * @return A valid activity class, or null if no suitable one is found
	...
	private static Class<? extends Activity> getErrorActivityClassWithIntentFilter(
			Context context) {
		Intent searchedIntent = new Intent().setAction(
				INTENT_ACTION_ERROR_ACTIVITY).setPackage(
				context.getPackageName());
		List<ResolveInfo> resolveInfos = context.getPackageManager()
				.queryIntentActivities(searchedIntent,
						PackageManager.GET_RESOLVED_FILTER);
	...
	private static void setLastCrashTimestamp(Context context, long timestamp) {
		context.getSharedPreferences(SHARED_PREFERENCES_FILE,
	...
	private static boolean hasCrashedInTheLastSeconds(Context context) {
		...
		return (lastTimestamp <= currentTimestamp && currentTimestamp
				- lastTimestamp < TIMESTAMP_DIFFERENCE_TO_AVOID_RESTART_LOOPS_IN_MILLIS);
	}

	/**
	 * Interface to be called when events occur, so they can be reported by the
	 * app as, for example, Google Analytics events.
	 */
